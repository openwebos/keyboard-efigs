...
#ifndef PHONE_KEYMAP_H
#define PHONE_KEYMAP_H
...
const UKey cKey_MorePopup = UKey(0x01200212);
...
class PhoneKeymap : public Mapper_IF
{
public:
    enum {
        cKeymapRows = 4,
    ...
    struct WKey {
        void set(UKey key, float weight = 1, const UKey * extended = NULL, const UKey * altExtended = NULL)
        {
            m_weight = weight, m_key = m_altkey = key, m_extended = extended, m_altExtended = altExtended;
        }
        void set(UKey key, UKey altkey, float weight = 1, const UKey * extended = NULL, const UKey * altExtended = NULL)
        {
            m_weight = weight, m_key = key, m_altkey = altkey, m_extended = extended, m_altExtended = altExtended;
        }
        void hide()
        {
            m_weight = 0, m_key = m_altkey = cKey_None, m_extended = m_altExtended = NULL;
        }
        bool operator !=(const WKey & rhs)
        {
            return m_weight != rhs.m_weight || m_key != rhs.m_weight || m_altkey != rhs.m_altkey || m_extended != rhs.m_extended || m_altExtended != rhs.m_altExtended;
        }
        ...
        const UKey *m_altExtended;
    };

    struct CustomKeys {
        CustomKeys(const WKey & beforeSpace, const WKey & leftSpace, const WKey & rightSpace, const WKey & afterSpace) : m_beforeSpace(beforeSpace), m_leftSpace(leftSpace), m_rightSpace(rightSpace), m_afterSpace(afterSpace) {}

        const WKey &m_beforeSpace;
        const WKey &m_leftSpace;
        const WKey &m_rightSpace;
        const WKey &m_afterSpace;
    };
    ...
    struct LayoutFamily {
        ...
        LayoutFamily(const char * name, const char * defaultLanguage, uint16_t primaryID, uint16_t secondaryID,
                     ...
                     const CustomKeys & plainKeys, const CustomKeys & symbolKeys, const CustomKeys & emailKeys, const CustomKeys & urlKeys, bool needNumLock,
                     Layout * layout);
        ...
        int m_space_x;   // x of space key on last row

        int m_return_x;   // x of return key
        int m_return_y;   // y of return key
        float m_return_key_weight;
        float m_symbol_key_weight;

        const CustomKeys & m_plainKeys;
        const CustomKeys & m_symbolKeys;
        const CustomKeys & m_emailKeys;
        const CustomKeys & m_urlKeys;
    ...
    bool setEditorState(const PalmIME::EditorState & editorState, bool updateTranslations = true);
